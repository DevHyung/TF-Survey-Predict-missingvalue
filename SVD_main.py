...
#118명 254개
data = genfromtxt('data.csv', delimiter=',',dtype=np.float32)
uim = np.nan_to_num(data)
...
if __name__ == "__main__":
    ...
    with graph.as_default():
        ...
        Sk = tf.diag(St)[0:nb_users, 0:nb_factors]
        Uk = Ut[:, 0:nb_factors]
        Vk = Vt[0:nb_users, :]
        # Compute Su and Si
    ...
    for i in range(1, 10):
        print('User {}: {}'.format(i, uim[i][best_items[0][i])])
